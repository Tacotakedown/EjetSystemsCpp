...
namespace E170Systems::UnitConversions {
    namespace Length {
        ...
        inline Kilometer metersToKilometers(const Meter m) { return m / meters_in_kilometer; }

        inline Meter kilometersToMeters(const Kilometer km) { return km * meters_in_kilometer; }

        inline Centimeter metersToCentimeters(const Meter m) { return m / meters_in_centimeter; }

        inline Meter centimetersToMeters(const Centimeter cm) { return cm * meters_in_centimeter; }

        inline Millimeter metersToMillimeters(const Meter m) { return m / meters_in_millimeter; }

        inline Meter millimetersToMeters(const Millimeter mm) { return mm * meters_in_millimeter; }

        inline Micrometer metersToMicrometers(const Meter m) { return m / meters_in_micrometer; }

        inline Meter micrometersToMeters(const Micrometer um) { return um * meters_in_micrometer; }

        inline Nanometer metersToNanometers(const Meter m) { return m / meters_in_nanometer; }

        inline Meter nanometersToMeters(const Nanometer nm) { return nm * meters_in_nanometer; }
        ...
        inline NauticalMile metersToNauticalMiles(const Meter m) { return m / meters_in_nautical_mile; }

        inline Meter nauticalMilesToMeters(const NauticalMile nm) { return nm * meters_in_nautical_mile; }

        inline AstronomicalUnit metersToAstronomicalUnits(const Meter m) { return m / meters_in_astronomical_unit; }

        inline Meter astronomicalUnitsToMeters(const AstronomicalUnit au) { return au * meters_in_astronomical_unit; }

        inline LightYear metersToLightYears(const Meter m) { return m / meters_in_light_year; }

        inline Meter lightYearsToMeters(const LightYear ly) { return ly * meters_in_light_year; }
    ...
    namespace Area {
        ...
        inline SquareInch squareMetersToSquareInches(const SquareMeter m2) { return m2 / square_meters_in_square_inch; }

        inline SquareMeter squareFeetToSquareMeters(const SquareFoot ft2) { return ft2 * square_meters_in_square_foot; }

        inline SquareFoot squareMetersToSquareFeet(const SquareMeter m2) { return m2 / square_meters_in_square_foot; }

        inline SquareCentimeter squareKilometersToSquareCentimeters(const SquareKilometer km2) {
            return squareMetersToSquareCentimeters(squareKilometersToSquareMeters(km2));
        }

        inline SquareKilometer squareCentimetersToSquareKilometers(const SquareCentimeter cm2) {
            return squareMetersToSquareKilometers(squareCentimetersToSquareMeters(cm2));
        }

        inline SquareMillimeter squareKilometersToSquareMillimeters(const SquareKilometer km2) {
            return squareMetersToSquareMillimeters(squareKilometersToSquareMeters(km2));
        }

        inline SquareKilometer squareMillimetersToSquareKilometers(const SquareMillimeter mm2) {
        ...
        inline SquareMillimeter squareCentimetersToSquareMillimeters(const SquareCentimeter cm2) {
            return squareMetersToSquareMillimeters(squareCentimetersToSquareMeters(cm2));
        }

        inline SquareCentimeter squareMillimetersToSquareCentimeters(const SquareMillimeter mm2) {
    ...
    namespace Volume {
        ...
        inline CubicMeter litersToCubicMeters(const Liter l) { return l * cubic_meters_in_liter; }

        inline Liter cubicMetersToLiters(const CubicMeter m3) { return m3 / cubic_meters_in_liter; }

        inline CubicMeter millilitersToCubicMeters(const Milliliter ml) { return ml * cubic_meters_in_milliliter; }

        inline Milliliter cubicMetersToMilliliters(const CubicMeter m3) { return m3 / cubic_meters_in_milliliter; }

        inline CubicMeter cubicInchesToCubicMeters(const CubicInch in3) { return in3 * cubic_meters_in_cubic_inch; }

        inline CubicInch cubicMetersToCubicInches(const CubicMeter m3) { return m3 / cubic_meters_in_cubic_inch; }

        inline CubicMeter cubicFeetToCubicMeters(const CubicFoot ft3) { return ft3 * cubic_meters_in_cubic_foot; }

        inline CubicFoot cubicMetersToCubicFeet(const CubicMeter m3) { return m3 / cubic_meters_in_cubic_foot; }

        inline CubicMeter gallonsToCubicMeters(const Gallon gal) { return gal * cubic_meters_in_gallon; }

        inline Gallon cubicMetersToGallons(const CubicMeter m3) { return m3 / cubic_meters_in_gallon; }

        inline CubicMeter quartsToCubicMeters(const Quart qt) { return qt * cubic_meters_in_quart; }

        inline Quart cubicMetersToQuarts(const CubicMeter m3) { return m3 / cubic_meters_in_quart; }

        inline CubicMillimeter cubicCentimetersToCubicMillimeters(const CubicCentimeter cm3) {
            return cubicMetersToCubicMillimeters(cubicCentimetersToCubicMeters(cm3));
        }

        inline CubicCentimeter cubicMillimetersToCubicCentimeters(const CubicMillimeter mm3) {
        ...
        inline CubicInch litersToCubicInches(const Liter l) { return cubicMetersToCubicInches(litersToCubicMeters(l)); }

        inline Liter cubicInchesToLiters(const CubicInch in3) {
            return cubicMetersToLiters(cubicInchesToCubicMeters(in3));
        }

        inline CubicFoot litersToCubicFeet(const Liter l) { return cubicMetersToCubicFeet(litersToCubicMeters(l)); }

        inline Liter cubicFeetToLiters(const CubicFoot ft3) { return cubicMetersToLiters(cubicFeetToCubicMeters(ft3)); }

        inline Gallon litersToGallons(const Liter l) { return cubicMetersToGallons(litersToCubicMeters(l)); }

        inline Liter gallonsToLiters(const Gallon gal) { return cubicMetersToLiters(gallonsToCubicMeters(gal)); }

        inline Quart litersToQuarts(const Liter l) { return cubicMetersToQuarts(litersToCubicMeters(l)); }

        inline Liter quartsToLiters(const Quart qt) { return cubicMetersToLiters(quartsToCubicMeters(qt)); }
        ...
        inline Quart cubicFeetToQuarts(const CubicFoot ft3) { return cubicMetersToQuarts(cubicFeetToCubicMeters(ft3)); }

        inline CubicFoot quartsToCubicFeet(const Quart qt) { return cubicMetersToCubicFeet(quartsToCubicMeters(qt)); }

        inline Quart gallonsToQuarts(const Gallon gal) { return cubicMetersToQuarts(gallonsToCubicMeters(gal)); }

        inline Gallon quartsToGallons(const Quart qt) { return cubicMetersToGallons(quartsToCubicMeters(qt)); }
    }
    ...
    namespace Mass {
        ...
        inline Gram kilogramsToGrams(const Kilogram kg) { return kg * grams_in_kilogram; }

        inline Kilogram tonnesToKilograms(const Tonne t) { return t * kilograms_in_tonne; }

        inline Tonne kilogramsToTonnes(const Kilogram kg) { return kg / kilograms_in_tonne; }

        inline Kilogram metricTonsToKilograms(const MetricTon mt) { return mt * kilograms_in_metric_ton; }

        inline MetricTon kilogramsToMetricTons(const Kilogram kg) { return kg / kilograms_in_metric_ton; }

        inline Kilogram poundsToKilograms(const Pound lb) { return lb / pounds_in_kilogram; }

        inline Pound kilogramsToPounds(const Kilogram kg) { return kg * pounds_in_kilogram; }

        inline Kilogram ouncesToKilograms(const Ounce oz) { return oz / ounces_in_kilogram; }

        inline Ounce kilogramsToOunces(const Kilogram kg) { return kg * ounces_in_kilogram; }

        inline Tonne gramsToTonnes(const Gram g) { return kilogramsToTonnes(gramsToKilograms(g)); }

        inline Gram tonnesToGrams(const Tonne t) { return kilogramsToGrams(tonnesToKilograms(t)); }

        inline MetricTon gramsToMetricTons(const Gram g) { return kilogramsToMetricTons(gramsToKilograms(g)); }

        inline Gram metricTonsToGrams(const MetricTon mt) { return kilogramsToGrams(metricTonsToKilograms(mt)); }

        inline Pound gramsToPounds(const Gram g) { return kilogramsToPounds(gramsToKilograms(g)); }

        inline Gram poundsToGrams(const Pound lb) { return kilogramsToGrams(poundsToKilograms(lb)); }

        inline Ounce gramsToOunces(const Gram g) { return kilogramsToOunces(gramsToKilograms(g)); }

        inline Gram ouncesToGrams(const Ounce oz) { return kilogramsToGrams(ouncesToKilograms(oz)); }

        inline MetricTon tonnesToMetricTons(const Tonne t) { return kilogramsToMetricTons(tonnesToKilograms(t)); }

        inline Tonne metricTonsToTonnes(const MetricTon mt) { return kilogramsToTonnes(metricTonsToKilograms(mt)); }

        inline Pound tonnesToPounds(const Tonne t) { return kilogramsToPounds(tonnesToKilograms(t)); }

        inline Tonne poundsToTonnes(const Pound lb) { return kilogramsToTonnes(poundsToKilograms(lb)); }

        inline Ounce tonnesToOunces(const Tonne t) { return kilogramsToOunces(tonnesToKilograms(t)); }

        inline Tonne ouncesToTonnes(const Ounce oz) { return kilogramsToTonnes(ouncesToKilograms(oz)); }

        inline Pound metricTonsToPounds(const MetricTon mt) { return kilogramsToPounds(metricTonsToKilograms(mt)); }

        inline MetricTon poundsToMetricTons(const Pound lb) { return kilogramsToMetricTons(poundsToKilograms(lb)); }

        inline Ounce metricTonsToOunces(const MetricTon mt) { return kilogramsToOunces(metricTonsToKilograms(mt)); }

        inline MetricTon ouncesToMetricTons(const Ounce oz) { return kilogramsToMetricTons(ouncesToKilograms(oz)); }
    ...
    namespace Time {
        ...
        inline Second millisecondsToSeconds(const Millisecond ms) { return ms * milliseconds_in_second; }

        inline Millisecond secondsToMilliseconds(const Second s) { return s / milliseconds_in_second; }

        inline Second minutesToSeconds(const Minute min) { return min * seconds_in_minute; }
